...
{
    if ($env:GITHUB_REF_TYPE -and $env:GITHUB_REF_TYPE.ToLowerInvariant() -eq "tag")
    {
        $kindFinal = "tag"
    }
    elseif ($env:GITHUB_EVENT_NAME -and $env:GITHUB_EVENT_NAME -like "pull_request*")
    {
        $kindFinal = "pr"
    }
    else
...
{
    if ($kindFinal -eq "tag" -and $env:GITHUB_REF_NAME)
    {
        $refFinal = $env:GITHUB_REF_NAME
    }
    elseif ($kindFinal -eq "pr" -and $env:SR_PR_NUMBER)
    {
        $refFinal = $env:SR_PR_NUMBER
    }
    elseif ($env:GITHUB_RUN_ID)
    {
        $refFinal = $env:GITHUB_RUN_ID
    }
}
...
if ($git -and (-not$commit))
{
    try
    {
        $commit = (& $git.Source -C $repo rev-parse HEAD 2> $null).Trim()
    }
    catch
    {
    }
