...
$branch = ""
if ($git) {
  try { $commit = (& $git.Source -C $repoPath rev-parse HEAD 2>$null).Trim() } catch {}
  try { $branch = (& $git.Source -C $repoPath rev-parse --abbrev-ref HEAD 2>$null).Trim() } catch {}
}

$ok = $true
foreach ($f in $findings) {
  if ($f.level -eq "error") { $ok = $false }
}
