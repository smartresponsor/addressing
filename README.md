...
**Address is data-only.** No Locator logic inside. No controllers needed if your platform autogenerates CRUD from
schema.
...
The applier updates normalized/geo fields, sets `validation_status=validated`, touches `updated_at` and appends
AddressUpdated to outbox.

## Dedupe key

- A canonical key is generated automatically when normalized fields are present (see `address_canonical_key()` and
  trigger `trg_address_dedupe_autofill`).
