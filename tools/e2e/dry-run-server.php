...
function reserveLoopbackPort(string $host): ?int
{
    $server = @stream_socket_server(sprintf('tcp://%s:0', $host), $errno, $errstr);
    if ($server === false) {
        return null;
    }
 ...
 */
function tryDryRunServer(string $host, int $port, array $env): array
{
...
$attempts = [];
foreach (dryRunHosts() as $host) {
    $port = reserveLoopbackPort($host);
    if ($port === null) {
        $attempts[] = [
            'host' => $host,
            'status' => 'blocked',
    ...
    $result = tryDryRunServer($host, $port, $env);
