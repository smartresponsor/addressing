...
BEFORE UPDATE ON address_entity
FOR EACH ROW EXECUTE FUNCTION address_touch_updated_at();
...
BEGIN
  IF NEW.dedupe_key IS NULL THEN
    k := address_canonical_key(NEW.line1_norm, NEW.city_norm, NEW.region_norm, NEW.postal_code_norm, NEW.country_code);
...
BEFORE INSERT OR UPDATE ON address_entity
FOR EACH ROW EXECUTE FUNCTION address_dedupe_autofill();
...
CREATE TRIGGER trg_address_audit_ins AFTER INSERT ON address_entity
FOR EACH ROW EXECUTE FUNCTION address_audit_func();

CREATE TRIGGER trg_address_audit_upd AFTER UPDATE ON address_entity
FOR EACH ROW EXECUTE FUNCTION address_audit_func();

CREATE TRIGGER trg_address_audit_del AFTER DELETE ON address_entity
FOR EACH ROW EXECUTE FUNCTION address_audit_func();
