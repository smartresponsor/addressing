...
function Find-RepoRoot([string]$startPath)
{
    ...
    while ($true)
    {
        ...
        if (Test-Path -LiteralPath $ghDir  -PathType Container)
        {
            return $p
        }

        $parent = Split-Path -Parent $p
        if ($parent -eq $p -or [string]::IsNullOrWhiteSpace($parent))
        {
            break
        }
        $p = $parent
...
if ($null -ne $policy)
{
    if ($null -ne $policy.excludeDir)
    {
        $ExcludeDir = Merge-Unique $ExcludeDir $policy.excludeDir
    }
    if ($null -ne $policy.excludeExt)
    {
        $ExcludeExt = Merge-Unique $ExcludeExt $policy.excludeExt
    }
}
...
foreach ($i in $delta)
{
    ...
    if (Should-ExcludeRelPath $i.path)
    {
        continue
    }
    $filtered += $i
