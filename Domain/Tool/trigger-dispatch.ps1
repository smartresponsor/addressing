# Copyright (c) 2025 Oleksandr Tishchenko / Marketing America Corp

param(
    [Parameter(Mandatory = $true)][string]$Url,
    [Parameter(Mandatory = $true)][ValidateSet("scan", "health", "doctor", "validate", "plan", "codex", "pr")][string]$Task,
    [string]$Ref = "master",
    [string]$Kind = "fix",
    [string]$Message = "agent update",
    [string]$Note = "",
    [string]$Kid = "K1"
)

$ErrorActionPreference = "Stop"

function toHex([byte[]]$Bytes)
{
    return ($Bytes | ForEach-Object { $_.ToString("x2") }) -join ""
}

function hMacSha256hex([string]$Secret, [string]$Data)
{
    $h = New-Object System.Security.Cryptography.HMACSHA256
    try
    {
        $h.Key = [System.Text.Encoding]::UTF8.GetBytes($Secret)
        $bytes = [System.Text.Encoding]::UTF8.GetBytes($Data)
        return (toHex ($h.ComputeHash($bytes)))
    }
    finally
    {
        $h.Dispose()
    }
}

$kidUp = $Kid.Trim().ToUpper()
if ($kidUp -notmatch '^K[1-9]\d*$')
{
    throw "Bad Kid: $Kid (expected K1, K2, ...)"
}

$envName = "SR_TRIGGER_SECRET_$kidUp"
$secret = (Get-Item "Env:$envName" -ErrorAction SilentlyContinue).Value
if (-not$secret)
{
    if ($kidUp -eq "K1" -and $env:SR_TRIGGER_SECRET)
    {
        $secret = $env:SR_TRIGGER_SECRET
    }
    else
    {
        throw "$envName is not set in environment"
    }
}

$ts = [int][DateTimeOffset]::UtcNow.ToUnixTimeSeconds()

$bodyObj = @{
    task = $Task
    ref = $Ref
    inputs = @{
        kind = $Kind
        message = $Message
        note = $Note
    }
}
$body = ($bodyObj | ConvertTo-Json -Depth 6 -Compress)

$sig = hMacSha256hex $secret "$ts.$body"

$headers = @{
    "X-SR-Kid" = $kidUp
    "X-SR-Timestamp" = "$ts"
    "X-SR-Signature" = $sig
    "Content-Type" = "application/json"
}

Write-Host "POST $Url task=$Task ref=$Ref kid=$kidUp"
$res = Invoke-RestMethod -Method Post -Uri $Url -Headers $headers -Body $body
$res | ConvertTo-Json -Depth 6
