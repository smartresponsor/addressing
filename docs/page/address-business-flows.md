...
**Process:** `AddressWriteService::update` calls `DoctrineAddressWriteRepository::update`, which updates the row and
...
**Process:** `AddressReadService::search` delegates to `DoctrineAddressReadRepository::findPage` and returns paged results.  
**Output:** `{ items: [...], nextCursor }`.

## Dedupe lookup

**Input:** normalized `dedupeKey`.  
**Process:** `AddressReadService::dedupe` calls `DoctrineAddressReadRepository::findByDedupeKey` to return a matching address (if
any).  
